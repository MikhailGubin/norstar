...
def get_employees_with_task_count():
    ...
    return User.objects.annotate(
        active_tasks_count=Count("executor_tasks", filter=Q(executor_tasks__status__in=active_statuses))
...
def get_important_tasks():
    ...
    active_statuses = get_active_statuses()
    return Task.objects.filter(status=Task.Status.CREATED, subtasks__status__in=active_statuses).distinct()
...
def get_potential_executors(task, employee_stats, min_load):
    ...
    if task.parent and task.parent.executor:
        ...
        executor_load = next(
            (stat["active_tasks_count"] for stat in employee_stats if stat["id"] == parent_executor.id), 0
        )
        if (
            executor_load <= min_load + 2
            and parent_executor != task.executor
            and parent_executor != task.owner
            and parent_executor.email not in added_emails
        ):
    ...
    least_loaded_employees = (
        ...
        .filter(active_tasks_count__lte=min_load + 2)
        .order_by("active_tasks_count")
    )
...
def format_important_task_data(task, potential_executors):
    ...
    return {
        ...
        "Возможные исполнители": [format_executor_name(executor) for executor in potential_executors],
    }
